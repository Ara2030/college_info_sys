    ...
    for i in range(6):
        ...
        obj, was_created = Student.objects.get_or_create(
            ...
            defaults={
                ...
                'education_doc_series': '77АБ',
                'education_doc_number': f'{random.randint(100000, 999999)}',
                'education_doc_date': date(birth.year + 9, 6, random.randint(15, 30)),
                'education_doc_org': 'Средняя общеобразовательная школа № 12',
                'social_card_number': f'SC-{random.randint(1000000, 9999999)}',
                'social_card_issued': date(enroll.year, 9, random.randint(1, 15)),
            })
        if was_created:
            created += 1

# --- Типы приказов (с шаблонами текста и номенклатурой дел) ---
order_types = [
    ('enroll', 'Зачисление', 'Приказ о зачислении',
     'В соответствии с Правилами приёма и на основании личных заявлений приказываю:\n{student}\nСчитать зачисленными с {date} в состав обучающихся ГБПОУ «Колледж».',
     '01-06', 'Приказы о зачислении обучающихся'),
    ('expel', 'Отчисление', 'Приказ об отчислении',
     'На основании решения педагогического совета и заявления приказываю:\n{student}\nОтчислить с {date} в связи с завершением обучения / по собственному желанию.',
     '01-10', 'Приказы об отчислении обучающихся'),
    ('transfer', 'Перевод', 'Приказ о переводе',
     'На основании заявлений и результатов промежуточной аттестации приказываю:\n{student}\nПеревести на следующий курс / в другую группу с {date}.',
     '01-07', 'Приказы о переводе обучающихся'),
    ('academic_leave', 'Академический отпуск', 'Приказ о предоставлении академического отпуска',
     'На основании заявления и медицинских показаний приказываю:\n{student}\nПредоставить академический отпуск с {date}.',
     '01-09', 'Приказы о предоставлении академических отпусков'),
    ('recover', 'Восстановление', 'Приказ о восстановлении',
     'На основании заявления и решения педагогического совета приказываю:\n{student}\nВосстановить в число обучающихся с {date}.',
     '01-11', 'Приказы о восстановлении обучающихся'),
]
for code, name, doc_name, template, file_code, file_title in order_types:
    OrderType.objects.get_or_create(
        code=code,
        defaults={'name': name, 'document_name': doc_name,
                  'template_text': template,
                  'file_code': file_code, 'file_title': file_title})
